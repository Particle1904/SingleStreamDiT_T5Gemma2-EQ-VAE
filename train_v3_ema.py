...
RESUME_FROM = "./checkpoints/checkpoint3_epoch_900.pt"  # Set to "checkpoints/v3/epoch_50.pt" to resume. Set to None for fresh start.
START_EPOCH = 901     # If resuming, set this manually (e.g. 50). If None, 0.

# TRAINING HYPERPARAMETERS
BATCH_SIZE = 8
LEARNING_RATE = 5e-5  # 1e-4 for fresh/aggressive, 5e-5 for fine-tuning
EPOCHS = 1200
...
TEXT_DROPOUT = 0.10   # % of dropout for text
EMA_DECAY = 0.999     # Standard for fine-tuning
...
@torch.no_grad()
def validate(model, vae, epoch, cache_dir, is_ema=False):
    model.eval()
    ...
    prefix = "EMA_" if is_ema else "RAW_"
    save_path = f"{SAMPLES_DIR}/{prefix}epoch_{epoch}.png"
...
def train():
    ...
    for epoch in range(START_EPOCH, EPOCHS + 1):
        ...
        if epoch % VALIDATE_EVERY == 0:
            validate(model, vae, epoch, CACHE_DIR, is_ema=False)
            validate(ema_model.module, vae, epoch, CACHE_DIR, is_ema=True)

        if epoch % SAVE_EVERY == 0:
            path = f"{CHECKPOINT_DIR}/epoch_{epoch}.pt"
            torch.save(model.state_dict(), path)
            ema_path = f"{CHECKPOINT_DIR}/ema_epoch_{epoch}.pt"
            torch.save(ema_model.module.state_dict(), ema_path)
            print(f"Checkpoints saved: {path} and {ema_path}")
